...
### AUTH-01. ✅ Log de acessos (SQLite) — CONCLUÍDO
SQLite WAL, tabela `access_log`, endpoints `/admin/log` e `/admin/users`. `DB_DIR` via env var.

### AUTH-02. ✅ Registo com verificação de e-mail — CONCLUÍDO
Tabela `users`, `/auth/register`, `/auth/verify/<token>`. E-mail em background thread via Flask-Mail/Resend.
Link de verificação corrigido para usar `request.url_root` (backend URL).
...
### Pendente — teste e2e em produção
- Configurar env vars no Render de produção (ver `docs/HOSTING_OPTIONS.md` para lista completa)
- Testar fluxo completo: registo → e-mail Resend → verificação → login → recuperação de palavra-passe
- Validar que o modal de sessão expirada aparece ao apagar cookies em produção
